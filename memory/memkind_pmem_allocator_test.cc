...
namespace rocksdb {
TEST(MemkindPmemAllocatorTest, Allocate) {
  MemkindPmemAllocator allocator;
  void* p;
  try {
...
TEST(MemkindPmemAllocatorTest, DatabaseBlockCache) {
  ...
  // Create database with block cache using MemkindPmemAllocator
  Options options;
  ...
  std::shared_ptr<Cache> cache = NewLRUCache(
      1024 * 1024, 6, false, false, std::make_shared<MemkindPmemAllocator>());
...
int main(int /*argc*/, char** /*argv*/) {
  printf(
      "Skip memkind_pmem_allocator_test as the required library memkind is "
      "missing.");
}
