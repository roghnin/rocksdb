...
namespace rocksdb {
TEST(PMDKTransAllocatorTest, Allocate) {
  PMDKTransAllocator allocator;
  void* p;
  try {
...
TEST(PMDKTransAllocatorTest, DatabaseBlockCache) {
  ...
  // Create database with block cache using PMDKTransAllocator
  Options options;
  std::string dbname = test::PerThreadDBPath("pmdk_transient_allocator_test");
  ASSERT_OK(DestroyDB(dbname, options));

  options.create_if_missing = true;
  std::shared_ptr<Cache> cache = NewLRUCache(
      1024 * 1024, 6, false, false, std::make_shared<PMDKTransAllocator>());
...
int main(int /*argc*/, char** /*argv*/) {
  printf(
      "Skip pmdk_transient_allocator_test as the required library pmdk is "
      "missing.");
}
