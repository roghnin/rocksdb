...
class NVMHandleTable {
 public:
  NVMHandleTable();
  ~NVMHandleTable();

  NVMHandle* Lookup(const Slice& key, uint32_t hash);
  NVMHandle* Insert(NVMHandle* h);
  NVMHandle* Remove(const Slice& key, uint32_t hash);
 ...
 private:
  ...
  NVMHandle** FindPointer(const Slice& key, uint32_t hash);
